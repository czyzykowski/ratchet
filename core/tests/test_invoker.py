...
class TestClaudeCodeInvoker:
    ...
    def test_trace_saved_to_store(self):
        ...
        assert result.trace_id == ctx.execution_id
        assert ctx.execution_id in store._traces
        trace = store._traces[ctx.execution_id]
        assert str(ctx.execution_id) in trace.content
        assert "COMPLETED: done" in trace.content

    def test_trace_id_matches_execution_id(self):
        ctx = _make_context()
        store = InMemoryStore()
        invoker = ClaudeCodeInvoker(store=store)
