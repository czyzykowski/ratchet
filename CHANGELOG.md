...
### Fixed
- Implementation pipeline now blocks task when Claude reports COMPLETED but produces an empty diff, preventing silent no-op transitions to QA
...
- Project page redesigned: features as card grid instead of flat table; task sections use colored status dots and left borders (no redundant per-row badges); header buttons split into create actions (primary) and navigation (ghost) with visual separator; new `.btn-ghost`, `.feature-card`, `.task-status-heading` CSS components

### Fixed
- Streamdown code blocks rendering as single horizontal line — added Tailwind polyfill CSS (`display: block` for line spans, `content-visibility` override) since app uses plain CSS not Tailwind
